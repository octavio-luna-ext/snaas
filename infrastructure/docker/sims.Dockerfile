...
# Build the Go app
RUN GOOS=linux GOARCH=amd64 go build -o bin/sims/sims cmd/sims/*.go
...
CMD /bin/sh -c "./sims --aws.id=$AWS_ID --aws.region=$AWS_REGION --aws.secret=$AWS_SECRET --aws.url=$AWS_URL --postgres.url=$POSTGRES_URL"
